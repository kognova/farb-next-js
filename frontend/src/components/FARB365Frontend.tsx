    ...
    const handleFileUpload = async (
        ...
        if (files) {
            ...
            try {
                ...
                setError(
                    "File upload or text extraction failed. Please try again."
                );
            } finally {
    ...
    const handleExportReport = async () => {
        ...
        if (element) {
            try {
                ...
                pdf.text(
                    `Engagement Letter: ${
                        letterFiles[0]?.name || "Not provided"
                    }`,
                    40,
                    70
                );
                pdf.text(
                    `Invoice: ${invoiceFiles[0]?.name || "Not provided"}`,
                    40,
                    90
                );
                pdf.text(
                    `Engagement Amendment: ${
                        amendmentFiles[0]?.name || "Not provided"
                    }`,
                    40,
                    110
                );
                ...
                scrollAreas.forEach((area, index) => {
                    (area as HTMLElement).style.height =
                        originalStyles[index].height;
                    (area as HTMLElement).style.overflow =
                                    ...
                                    >
                                        ...
                                        <div className="mt-1 flex items-center space-x-2">
                                            <Input
                                                ...
                                                disabled={
                                                    isExtracting[
                                                        type as keyof typeof isExtracting
                                                    ]
                                                }
                                            />
                                            <Button
